...
export const LockedChestOne = ({ room }) => {
  ...
  const openChest = () => {
    if (!checkedKeys) {
      ...
      if (keyRoll.total <= parseInt(keys)) {
        setResult("You have the key to this chest. It opens. TC");
        setKeys(-1);
        setIsOpened(true);
        setCheckedKeys(true);
    ...
    if (checkedKeys && parseInt(picks) > 0) {
      ...
      if (
        roll.total <=
        parseInt(dex) + parseInt(locks) + parseInt(lucky) - 20
      ) {
        setResult(
          `You successfully pick the lock and open the chest (${roll.total}). TC`
        );
        setIsOpened(true);
        room.geoFeatureComplete = true;
        useBoundStore.setState({ mapTiles: tiles });
  ...
  return (
    !complete && (
      <>
        <p>
          <strong>Locked Chest (26-29)</strong> A large wooden chest banded with
          golden trim sits proudly waiting to be opened. If you do not have the
