...
export const LockedChestThree = ({ room }) => {
  ...
  const openChest = () => {
    if (!checkedKeys) {
      ...
      if (keyRoll.total <= parseInt(keys)) {
        setResult("You have the key to this chest. It opens. TA");
        setKeys(-1);
        setIsOpened(true);
        setCheckedKeys(true);
    ...
    if (checkedKeys && parseInt(picks) > 0) {
      ...
      if (
        roll.total <=
        parseInt(dex) + parseInt(locks) + parseInt(lucky) - 10
      ) {
        setResult(
          `You successfully pick the lock and open the chest (${roll.total}). TA`
        );
        setIsOpened(true);
        room.geoFeatureComplete = true;
        useBoundStore.setState({ mapTiles: tiles });
  ...
  return (
    !complete && (
      <>
        <p>
          <strong>Locked Chest (55-58)</strong> A large wooden chest banded with
          iron trim sits proudly waiting to be opened. If you do not have the
