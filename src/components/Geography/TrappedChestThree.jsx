...
export const TrappedChestThree = ({ room }) => {
  ...
  const openChest = () => {
    ...
    if (roll.total <= parseInt(dex) + parseInt(traps) + parseInt(lucky) - 10) {
      setResult(
        `You successfully disarm the trap and open the chest (${roll.total}). TA+10`
      );
      setIsOpened(true);
      room.geoFeatureComplete = true;
      useBoundStore.setState({ mapTiles: tiles });
    } else {
      setResult(`You fail to disarm the trap (${roll.total}). Time +1, -3 HP.`);
      passTime();
      setAdjustedHP(-3);
    }
  };
  ...
  return (
    !complete && (
      <>
        <p>
          <strong>Trapped Chest (45-48)</strong> A large wooden chest banded
          with iron trim sits proudly waiting to be opened. You may try and open
