...
export const TrappedChestOne = ({ room }) => {
  ...
  const openChest = () => {
    ...
    if (roll.total <= parseInt(dex) + parseInt(traps) + parseInt(lucky) - 20) {
      setResult(
        `You successfully disarm the trap and open the chest (${roll.total}). TC+10`
      );
      setIsOpened(true);
      room.geoFeatureComplete = true;
      useBoundStore.setState({ mapTiles: tiles });
    } else {
      setResult(`You fail to disarm the trap (${roll.total}). Time +1, -5 HP.`);
      passTime();
      setAdjustedHP(-5);
    }
  };
  ...
  return (
    !complete && (
      <>
        <p>
          <strong>Trapped Chest (19-22)</strong> A large wooden chest banded
          with golden trim sits proudly waiting to be opened. You may try and
          open the chest by making a TRAPPED CHEST test as many times as you
          wish until it is opened.
          <br />
          {!isOpened && (
