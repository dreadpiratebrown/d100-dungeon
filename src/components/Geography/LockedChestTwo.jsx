...
export const LockedChestTwo = ({ room }) => {
  ...
  const openChest = () => {
    if (!checkedKeys) {
      ...
      if (keyRoll.total <= parseInt(keys)) {
        setResult("You have the key to this chest. It opens. TB");
        setKeys(-1);
        setIsOpened(true);
        setCheckedKeys(true);
    ...
    if (checkedKeys && parseInt(picks) > 0) {
      ...
      if (
        roll.total <=
        parseInt(dex) + parseInt(locks) + parseInt(lucky) - 15
      ) {
        setResult(
          `You successfully pick the lock and open the chest (${roll.total}). TB`
        );
        setIsOpened(true);
        room.geoFeatureComplete = true;
        useBoundStore.setState({ mapTiles: tiles });
  ...
  return (
    !complete && (
      <>
        <p>
          <strong>Locked Chest (43)</strong> A large wooden chest banded with
          silver trim sits proudly waiting to be opened. If you do not have the
